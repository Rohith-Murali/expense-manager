...
export const getAll = async (req, res) => {
  ...
  const paymentTypes = await paymentTypeService.getByAccount(req.user._id, req.params.accountId, type);
  res.json({ success: true, data: paymentTypes });
};

export const getById = async (req, res) => {
  const paymentType = await paymentTypeService.getById(req.user._id, req.params.id, req.params.accountId);
  res.json({ success: true, data: paymentType });
};

export const update = async (req, res) => {
  const paymentType = await paymentTypeService.update(req.user._id, req.params.id, req.params.accountId, req.body);
