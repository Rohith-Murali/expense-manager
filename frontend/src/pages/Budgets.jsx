...
const Budgets = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const analyticsList = Array.isArray(a)
        ? a
        : a?.categories || a?.data || [];
  ...
  const handleCreate = async () => {
    ...
    if (!account?.monthlyBudget || account.monthlyBudget <= 0) {
      addToast({
        type: 'error',
        message:
          'Please set the account total monthly budget first',
      });
    ...
    try {
      const existingTotal = (budgets || []).reduce(
        (s, x) => s + (Number(x.amount) || 0),
        0
      );

      const proposed =
        existingTotal + Number(newAmount || 0);

      if (
        account?.monthlyBudget &&
        account.monthlyBudget > 0 &&
        proposed > account.monthlyBudget
      ) {
        addToast({
          type: 'error',
  ...
  const totalCategoryBudget = budgets.reduce(
    (sum, item) => sum + (Number(item.amount) || 0),
    0
  );

  const totalRemaining = account?.monthlyBudget
    ? account.monthlyBudget - totalCategoryBudget
    : 0;
        ...
        <div className="mb-8 rounded-3xl overflow-hidden shadow-sm border border-slate-200 bg-white">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div>
                ...
                <p className="mt-3 text-slate-600 max-w-2xl">
                  Manage monthly spending limits and
                  category-wise allocations in one place.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-slate-50 border-t border-slate-200">
            ...
            <div className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm">
              ...
              <p
                className={`mt-3 text-3xl font-bold ${totalRemaining < 0
                  ? 'text-red-400'
                  : 'text-emerald-300'
                  }`}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {budgets.map((b) => {
                  ...
                  return (
                    <div
                      key={b._id}
                      className={`rounded-3xl border p-5 shadow-sm ${remaining < 0
